...
class ViTForgeryDetectorLoRA(nn.Module):
    def __init__(self, lora_rank=8, lora_alpha=16):
        ...
        # 1. 冻结所有主干参数 (Backbone Frozen)
        print("Freezing ViT backbone parameters...")
        for param in self.backbone.parameters():
            param.requires_grad = False
        
        # 2. 禁用 LoRA 注入 (Disabled LoRA)
        # 原代码：self.lora_params = inject_lora_to_linear(...)
        print("Skipping LoRA injection. Training in Linear Probe mode (Classifier only).")
        self.lora_params = [] 
        
        self.feature_dim = self.backbone.config.hidden_size 
        
        # 3. 分类器 (Classifier) - 这是唯一可训练的部分
        self.classifier = nn.Sequential(
            nn.Linear(self.feature_dim, 1)
        )
    
    def forward(self, x):
        # 显式使用 no_grad 确保 backbone 不计算梯度（虽然 requires_grad=False 已经够了，这更安全）
        with torch.no_grad():
...
def main():
    ...
    CFG = {
        'seed': 42,
        'batch_size': 32,
        'lr': 5e-5, # 修改: 只训练分类头通常可以使用稍微大一点的学习率 (例如 1e-3 或 5e-4)
        'epochs': 100,
        'lora_rank': 8,   # 未使用
        'lora_alpha': 16, # 未使用
        ...
        'log_dir': './logs_vit',     # 修改: 文件夹名以区分实验
        'ckpt_dir': './checkpoints_vit' # 修改: 文件夹名以区分实验
    }
    ...
    logger.info("="*70)
    logger.info("ViT-Large Frozen Backbone + Linear Head Training")
    logger.info("="*70)
    ...
    logger.info(f"  Trainable (Head Only): {trainable_params:,}")
    logger.info(f"  Ratio: {trainable_params/total_params*100:.4f}%")
    
    # 优化器与损失
    criterion = nn.BCEWithLogitsLoss()
    # 这里的 get_trainable_params 现在只返回 classifier 的参数
    ...
    best_acc = 0.0
    best_epoch = 0
    best_model_path = os.path.join(CFG['ckpt_dir'], 'best_model_frozen_linear.pth')
