...
def inject_lora_to_linear(model, rank=8, alpha=16, target_modules=['query', 'key', 'value']):
    """
    HuggingFace ViT 的 Attention 层通常命名为: query, key, value
    """
    lora_params = []
...
class ViTForgeryDetectorLoRA(nn.Module):
    def __init__(self, lora_rank=8, lora_alpha=16):
        ...
        # 2. 仿照 DINOv2 风格使用 nn.Sequential
        # 结构: [Feature_Dim] -> MLP(Feature_Dim -> 4x -> Feature_Dim) -> Linear(Feature_Dim -> 1)
...
def main():
    ...
    best_acc = 0.0
    best_epoch = 0
    best_model_path = os.path.join(CFG['ckpt_dir'], 'best_model_vit_lora_transmlp.pth')
