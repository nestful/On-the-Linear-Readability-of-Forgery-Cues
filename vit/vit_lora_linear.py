...
def inject_lora_to_linear(model, rank=8, alpha=16, target_modules=['query', 'key', 'value']):
    """
    HuggingFace ViT 的 Attention 层通常命名为:
    - query
    - key
    - value
    (注意：不同于 BLIP 的 qkv 融合层，这里通常是分开的)
    """
    lora_params = []
...
class ViTForgeryDetectorLoRA(nn.Module):
    def __init__(self, lora_rank=8, lora_alpha=16):
        super().__init__()
        # 修改点 2: 加载 google/vit-large-patch16-224 (与 BLIP Large 规格一致)
        ...
        # 修改点 3: ViT 的 QKV 是分开的线性层，且要求只微调 QKV
    ...
    def forward(self, x):
        ...
        # [修改点 2] 使用 last_hidden_state 的第0个token ([CLS])
        # last_hidden_state 形状: [Batch, Sequence_Length, Hidden_Size]
        # 取 [:, 0, :] 得到 [Batch, Hidden_Size]
        # 通常使用 pooler_output (对应 [CLS] token 经过 MLP) 进行分类
...
def main():
    ...
    CFG = {
        ...
        'log_dir': './logs_vit',     # 修改: 文件夹名
        'ckpt_dir': './checkpoints_vit' # 修改: 文件夹名
    }
    ...
    logger.info("="*70)
    logger.info("ViT-Large LoRA Fine-tuning for Forgery Detection")
    logger.info("="*70)
    logger.info(f"Log file: {log_file}")
    logger.info(f"Device: {device}")
    logger.info(f"Config: {json.dumps(CFG, indent=2)}")
    
    # 修改点 6: ViT 标准归一化参数 (Mean=0.5, Std=0.5)
    norm_mean = (0.5, 0.5, 0.5)
    norm_std = (0.5, 0.5, 0.5)
    ...
    best_acc = 0.0
    best_epoch = 0
    best_model_path = os.path.join(CFG['ckpt_dir'], 'best_model_vit_lora_linear.pth')
