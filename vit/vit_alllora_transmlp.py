...
class MLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        # 结构: [n_embd] -> Linear -> [4*n_embd] -> GELU -> Linear -> [n_embd] -> Dropout
...
class ViTForgeryDetectorLoRA(nn.Module):
    def __init__(self, lora_rank=8, lora_alpha=16):
        ...
        print("Targets: QKV + MLP") # 打印提示信息
        
        # ====================== 修改核心 ======================
        # 在 target_modules 中加入 MLP 相关的层名称：
        # 'intermediate.dense': MLP 的升维层
        # 'output.dense': MLP 的降维层 (也会匹配到 Attention 的 Output 投影层，这通常是有益的)
...
def main():
    ...
    logger.info("="*70)
    logger.info("ViT-Large LoRA Fine-tuning (QKV + MLP)")
    logger.info("="*70)
    ...
    best_acc = 0.0
    best_epoch = 0
    best_model_path = os.path.join(CFG['ckpt_dir'], 'best_model_vit_alllora_transmlp.pth')
