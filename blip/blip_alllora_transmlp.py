...
class BLIPForgeryDetectorLoRA(nn.Module):
    def __init__(self, lora_rank=8, lora_alpha=16, mode='all'):
        ...
        # 获取隐藏层维度 (BLIP-Base 为 768)
        self.feature_dim = self.backbone.config.hidden_size
        print(f"✅ Feature Dimension: {self.feature_dim}")
        
        # ========== 修改部分：使用自定义 MLP 作为分类头 ==========
        # 定义 MLP 所需的配置类
        class MLPConfig:
        ...
        # 构建分类器：先经过您的 MLP 模块提取特征，再经过一层 Linear 映射到 1 个输出
        # 因为提供的 MLP 模块输入输出维度相同 (n_embd -> n_embd)，所以必须加一层 Linear(n_embd, 1) 做二分类
        self.classifier = nn.Sequential(
            MLP(self.mlp_config),         # Custom
            nn.Linear(self.feature_dim, 1) # Final Projection: 768 -> 1
        )
        # =======================================================
...
def main():
    CFG = {
        ...
        'log_dir': './logs_blip',       # 日志目录
        'ckpt_dir': './checkpoints_blip' # 权重保存目录
    }
    ...
    logger.info("="*70)
    logger.info("BLIP + LoRA + Custom MLP Head for Forgery Detection")
    logger.info("="*70)
    logger.info(f"Config: {json.dumps(CFG, indent=2)}")
    
    # 归一化 (BLIP 沿用 CLIP 的参数)
    ...
    best_acc = 0.0
    best_epoch = 0
    best_model_path = os.path.join(CFG['ckpt_dir'], 'best_model_blip_alllora_transmlp.pth')
