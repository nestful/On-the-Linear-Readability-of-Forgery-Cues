...
class BLIPForgeryDetectorLoRA(nn.Module):
    def __init__(self, lora_rank=8, lora_alpha=16):
        ...
        # ========== 修改处：仿照 DINOv2 风格使用 nn.Sequential ==========
...
def main():
    ...
    model = BLIPForgeryDetectorLoRA(lora_rank=CFG['lora_rank'], lora_alpha=CFG['lora_alpha']).to(device)
    ...
    best_acc = 0.0
    best_epoch = 0
    best_model_path = os.path.join(CFG['ckpt_dir'], 'best_model_blip_lora_transmlp.pth')
