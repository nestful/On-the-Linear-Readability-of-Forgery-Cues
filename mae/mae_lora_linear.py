...
def inject_lora_to_linear(model, rank=8, alpha=16, target_modules=['query', 'key', 'value']):
    ...
    - encoder.layer.x.attention.output.dense (Projection)
    """
    lora_params = []
...
class MAEForgeryDetectorLoRA(nn.Module):
    def __init__(self, lora_rank=8, lora_alpha=16):
        ...
        # MAE (ViT) 的注意力层通常命名为 query, key, value 以及输出投影层
        # 我们在这里注入 LoRA，保证 QKV 和 Projection 都被微调
        ...
        self.feature_dim = self.backbone.config.hidden_size # 1024 for Large
...
def test_model(model_path, test_dir, log_dir, device):
    ...
    # 准备测试数据 - 修改3：使用标准 ImageNet 归一化参数
    test_transform = transforms.Compose([
        transforms.Resize((224, 224), interpolation=transforms.InterpolationMode.BICUBIC),
        transforms.ToTensor(),
        # MAE 使用标准的 ImageNet 均值和方差
...
def main():
    ...
    # 修改4：使用标准 ImageNet 归一化参数 (MAE 默认设置)
    ...
    best_acc = 0.0
    best_epoch = 0
    best_model_path = os.path.join(CFG['ckpt_dir'], 'best_model_mae_lora_linear.pth')
