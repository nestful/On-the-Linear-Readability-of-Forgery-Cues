...
def inject_lora_to_linear(model, rank=8, alpha=16, target_modules=['query', 'key', 'value']):
    """
    修改默认 target_modules 以适配 ViT/MAE 结构
    """
    lora_params = []
...
class TrainingLogger:
    def __init__(self, log_dir):
        ...
        self.history = {'train_loss': [], 'train_acc': [], 'val_loss': [], 'val_acc': [], 'val_auc': [], 'learning_rates': []}
    ...
    def save_final(self, best_epoch, best_acc, test_results=None):
        results = {'best_epoch': best_epoch, 'best_val_acc': float(best_acc), 'training_history': self.history}
        if test_results:
...
# ==================== 自定义 MLP 模块 (保持不变) ====================
class MLPConfig:
    """简单的配置类，用于适配 MLP 的初始化"""
    def __init__(self, n_embd, bias=True, dropout=0.0):
        self.n_embd = n_embd
        self.bias = bias
...
# ==================== 模型定义 (MAE 修改版) ====================
class MAEForgeryDetectorLoRA(nn.Module):
    def __init__(self, lora_rank=8, lora_alpha=16):
        super().__init__()
        print("Loading MAE Vision Model (facebook/vit-mae-large)...")
        # 1. 加载 MAE Large 模型 (Hidden Size = 1024, 与 BLIP Large Vision Model 一致)
        ...
        # MAE/ViT 的注意力层通常命名为 query, key, value
        # 我们在这里注入 LoRA，同时加入 attention.output.dense 以保证投影层也被微调
        ...
        mlp_config = MLPConfig(
            n_embd=self.feature_dim,
            bias=True,
            dropout=0.1 
        )
        
        # 分类器结构保持不变: [1024] -> MLP(1024->4096->1024) -> Linear(1024->1)
    ...
    def forward(self, x):
        # MAE 输出: BaseModelOutput (last_hidden_state: [Batch, SeqLen, Hidden])
        outputs = self.backbone(pixel_values=x)
        
        # 提取 CLS Token (索引为0) 对应的特征
        # MAE/ViT 是 Encoder，第一个 token 是 [CLS]
        features = outputs.last_hidden_state[:, 0, :] # [Batch, 1024]

        # 通过 nn.Sequential 定义的分类器 (MLP + Linear)
...
def evaluate(model, loader, criterion, device, logger, prefix='Val'):
    ...
    logger.info(f"{prefix} Loss: {metrics['loss']:.4f} | {prefix} Acc: {metrics['acc']:.4f} | AUC: {metrics['auc']:.4f}")
    return metrics
...
def test_model(model_path, test_dir, log_dir, device):
    ...
    # 实例化 MAE 模型
    model = MAEForgeryDetectorLoRA().to(device)
    checkpoint = torch.load(model_path, map_location=device)
    model.load_state_dict(checkpoint)
    logger.info(f"Model loaded from {model_path}")
    
    # 修改：使用 ImageNet 标准归一化参数 (MAE 默认)
...
def main():
    CFG = {
        ...
        'log_dir': './logs_mae', # 文件夹名改为 mae
        'ckpt_dir': './checkpoints_mae' # 文件夹名改为 mae
    }
    ...
    logger.info("="*70)
    logger.info("MAE (Large) Vision LoRA Fine-tuning for Forgery Detection")
    logger.info("="*70)
    ...
    # 模型实例化 - 使用 MAE 类
    model = MAEForgeryDetectorLoRA(lora_rank=CFG['lora_rank'], lora_alpha=CFG['lora_alpha']).to(device)
    ...
    best_acc = 0.0
    best_epoch = 0
    best_model_path = os.path.join(CFG['ckpt_dir'], 'best_model_mae_lora_transmlp.pth')
