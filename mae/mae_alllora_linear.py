...
def inject_lora_to_linear(model, rank=8, alpha=16, target_modules=['query', 'value']):
    lora_params = []
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, nn.Linear) and is_target:
            # print(f"Injecting LoRA to: {name}") # 调试时可打开
...
class MAEForgeryDetectorLoRA(nn.Module):
    def __init__(self, lora_rank=8, lora_alpha=16):
        super().__init__()
        # 使用 facebook/vit-mae-large (与 BLIP Large 同为 Large 规模，Hidden=1024)
        ...
        # ================== LoRA 目标层配置 (QKV + MLP) ==================
        # HuggingFace ViT/MAE 的 Linear 层命名规则：
        # Attention: 'query', 'key', 'value'
        # Attention Projection: 'attention.output.dense'
        # MLP (FFN): 'intermediate.dense' (FC1), 'output.dense' (FC2)
        # 注意：'output.dense' 既存在于 Attention Output 也在 MLP Output，
        # 我们可以通过包含 dense 来同时覆盖投影层和 MLP 第二层。
        
        self.target_modules = [
            'query', 'key', 'value',       # Attention QKV
            'intermediate.dense',          # MLP Layer 1
            'output.dense'                 # MLP Layer 2 & Attention Output Projection
        ]
        # ===============================================================
        ...
        print(f"✅ 微调目标层 (QKV + MLP): {self.target_modules}")

        # 获取隐藏层维度 (MAE Large 为 1024)
    ...
    def forward(self, x):
        # MAE 输出: BaseModelOutputWithPooling (虽然叫WithPooling但通常MAE只有last_hidden_state)
        outputs = self.backbone(pixel_values=x)
        
        # MAE 是纯 Encoder，输出维度 [Batch, SeqLen (197), Hidden (1024)]
        # 取第一个 token [CLS] 作为图像的整体特征表示
...
def main():
    ...
    CFG = {
        ...
        'log_dir': './logs_mae',       # 修改目录名
        'ckpt_dir': './checkpoints_mae' # 修改目录名
    }
    ...
    logger.info("="*70)
    logger.info("MAE (ViT-Large) LoRA Fine-tuning for Forgery Detection")
    logger.info("="*70)
    logger.info(f"Log file: {log_file}")
    logger.info(f"Device: {device}")
    logger.info(f"Config: {json.dumps(CFG, indent=2)}")
    
    # 2. 数据增强 (修改：使用 ImageNet 标准 Mean/Std)
    ...
    best_epoch = 0
    # 修改保存路径名
    best_model_path = os.path.join(CFG['ckpt_dir'], 'best_model_mae_alllora_linear.pth')
