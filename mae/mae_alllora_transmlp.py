...
class MAEForgeryDetectorLoRA(nn.Module):
    def __init__(self, lora_rank=8, lora_alpha=16):
        ...
        # Attention QKV: 'query', 'key', 'value'
        # MLP (Feed Forward): 'intermediate.dense' (第1层), 'output.dense' (第2层)
        # 注意: 'output.dense' 同时存在于 Attention Output 和 MLP Output，这里一并微调
        self.target_modules = [
            'query', 'key', 'value',       # Attention QKV
            'intermediate.dense',          # MLP Layer 1 (FC1)
            'output.dense'                 # MLP Layer 2 (FC2) & Attn Projection
        ]
        ...
        self.mlp_config = MLPConfig(n_embd=self.feature_dim, bias=True, dropout=0.1)
        
        self.classifier = nn.Sequential(
            MLP(self.mlp_config),          # 您的自定义 MLP
            nn.Linear(self.feature_dim, 1) # 最终映射到 1 维 logits
        )
        # ==================================
...
def main():
    CFG = {
        ...
        'log_dir': './logs_mae',       # 修改文件夹名
        'ckpt_dir': './checkpoints_mae' # 修改文件夹名
    }
    ...
    logger.info("="*70)
    logger.info("MAE (ViT-Large) + LoRA + Custom MLP Head")
    logger.info("="*70)
    logger.info(f"Config: {json.dumps(CFG, indent=2)}")
    
    # 归一化: 使用 ImageNet 标准参数
    ...
    best_epoch = 0
    # 修改模型保存名
    best_model_path = os.path.join(CFG['ckpt_dir'], 'best_model_mae_alllora_transmlp.pth')
