...
class MLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        # n_embd: 输入维度 (对于 DINOv2 ViT-L 是 1024)
        # 4 * n_embd: 隐藏层维度 (4096)
...
def inject_lora_to_linear(model, rank=8, alpha=16, target_modules=['qkv', 'proj']):
    lora_params = []
...
class DINOv2ForgeryDetectorLoRA(nn.Module):
    def __init__(self, lora_rank=8, lora_alpha=16, mlp_dropout=0.1):
        super().__init__()
        ...
        # 构建 MLP 配置
        # DINOv2 ViT-L 输出是 1024 维
        config = MLPConfig(n_embd=self.feature_dim, dropout=mlp_dropout, bias=True)
        
        # 分类器修改：先经过 Transformer 风格的 MLP，再映射到 1 维进行二分类
        # 结构: [1024] -> MLP(1024->4096->1024) -> Linear(1024->1)
...
# ==================== 测试函数 ====================
def test_model(model_path, test_dir, log_dir, device, mlp_dropout):
    """在测试集上评估模型"""
    logger = logging.getLogger(__name__)
    
    # 加载模型 (注意传入同样的参数)
    model = DINOv2ForgeryDetectorLoRA(mlp_dropout=mlp_dropout).to(device)
...
def main():
    ...
    logger.info("="*70)
    logger.info("DINOv2 LoRA Fine-tuning for Forgery Detection (with MLP Head)")
    logger.info("="*70)
    ...
    # worker_init_fn 进一步保证DataLoader多进程下的随机性
    ...
    best_acc = 0.0
    best_epoch = 0
    best_model_path = os.path.join(CFG['ckpt_dir'], 'best_model_lora_transmlp.pth')
    ...
    # 注意这里也传了 dropout 参数，虽然测试时dropout是不激活的，但需要用来初始化结构
    test_results = test_model(best_model_path, CFG['test_path'], CFG['log_dir'], device, CFG['mlp_dropout'])
