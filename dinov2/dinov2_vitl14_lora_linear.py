...
#这里为何单用一个代码来写就是为了方便测试linear的时候方便导入，虽然只改一下dinov2_vitl14_lora一点代码就可以了。
...
def main():
    ...
    logger.info("="*70)
    logger.info("DINOv2 LoRA Fine-tuning for Forgery Detection")
    logger.info("="*70)
    ...
    best_acc = 0.0
    best_epoch = 0
    best_model_path = os.path.join(CFG['ckpt_dir'], 'best_model_twoclass_cathorse.pth')
