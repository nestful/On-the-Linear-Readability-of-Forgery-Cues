...
from types import SimpleNamespace # 用于创建简易的 config 对象
...
class CLIPForgeryDetectorLoRA(nn.Module):
    def __init__(self, lora_rank=8, lora_alpha=16, mode='attention'):
        super().__init__()
        ...
        # CLIP ViT-L/14 的 hidden_size 是 1024
        self.feature_dim = 1024
        
        # ================== 修改开始：替换原来的 Classifier ==================
        
        # 1. 配置 MLP 参数
        self.mlp_config = SimpleNamespace(
            n_embd=self.feature_dim,  # 1024
            bias=True,
            dropout=0.1  # 推荐设为 0.1 或 0.3
        )
        ...
        # 3. 初始化最终预测头 (Prediction Head)
        # 因为 MLP 输出还是 1024，我们需要把它压缩到 1 用于二分类
        self.prediction_head = nn.Linear(self.feature_dim, 1)
    ...
    def forward(self, x):
        ...
        # 1. 通过 MLP 进行特征变换
        # 这里的 features 经过了 1024->4096->1024 的变换
        enhanced_features = self.mlp(features)
        
        # [可选技巧] 残差连接 (Residual Connection)
        # 如果你发现训练很难收敛，可以把下面这行取消注释，变成 ResNet 结构：
        # enhanced_features = features + enhanced_features 
        
        # 2. 通过预测头得到分数
        logits = self.prediction_head(enhanced_features)
...
def test_model(model_path, test_dir, log_dir, device):
    ...
    # 更改：使用 CLIP 类
    model = CLIPForgeryDetectorLoRA().to(device)
...
def main():
    ...
    logger.info("="*70)
    logger.info("CLIP (ViT-L/14) LoRA Fine-tuning for Forgery Detection")
    logger.info("="*70)
    logger.info(f"Log file: {log_file}")
    logger.info(f"Device: {device}")
    logger.info(f"Config: {json.dumps(CFG, indent=2)}")
    
    # 2. 更改数据增强中的 Normalize
    # CLIP 官方推荐的 Mean 和 Std (不同于 ImageNet)
    ...
    best_acc = 0.0
    best_epoch = 0
    best_model_path = os.path.join(CFG['ckpt_dir'], 'best_model_clip_lora_transmlp.pth')
