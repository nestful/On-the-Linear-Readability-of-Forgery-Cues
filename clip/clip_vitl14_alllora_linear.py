...
def inject_lora_to_linear(model, rank=8, alpha=16, target_modules=['q_proj', 'v_proj']): #这里的qv只是默认值，如果不告诉调什么参数就只调qv
    lora_params = []
...
class CLIPForgeryDetectorLoRA(nn.Module):
    def __init__(self, lora_rank=8, lora_alpha=16,mode='all'):
        ...
        # 加载 HuggingFace 的 CLIP Vision Model (ViT-L/14)
        # 第一次运行会自动下载权重
        ...
        # ================== 核心修改 ==================
        if mode == 'attention':
            # 方案 A: 仅微调注意力层 (类似你之前的 DINOv2 代码)
            # 这种模式参数量少，训练稍快
            self.target_modules = ['q_proj', 'k_proj', 'v_proj', 'out_proj']
        
        elif mode == 'all':
            # 方案 B: 微调 注意力 + MLP (推荐方案)
            # 这种模式通常效果最好，适合伪造检测这种需要重新学习特征的任务 ()
        ...
        # CLIP ViT-L/14 的 hidden_size 也是 1024
    ...
    def forward(self, x):
        # HuggingFace CLIP 输出是一个对象
        outputs = self.backbone(pixel_values=x)
        
        # pooler_output 是 [CLS] token 经过投影后的特征 (batch_size, 1024)
        # last_hidden_state 是 (batch_size, seq_len, 1024)
...
def main():
    ...
    best_acc = 0.0
    best_epoch = 0
    best_model_path = os.path.join(CFG['ckpt_dir'], 'best_model_clip_alllora_linear.pth')
